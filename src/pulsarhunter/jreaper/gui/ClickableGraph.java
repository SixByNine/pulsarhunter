...
public class ClickableGraph extends Plot {
    ...
    public ClickableGraph(final Cand[][] data,final PlotType plotType ,MainView master) {
        ...
        initComponents();
        this.master = master;
        changeData(data,plotType,true);
        
    }
    
    public void changeData(final Cand[][] data,final PlotType plotType,final boolean showKnown){
        galactic = false;
                            ...
                            for(int i = 0;i<data[j].length;i++){
                                if(data[j][i].getCandClass() >= 0 ){
                                    if(master.show(data[j][i].getHarmonicType())){
                                        ...
                                        else  addPoint(255-plotType.getZval(data[j][i]),plotType.getXval(data[j][i]),plotType.getYval(data[j][i]),false);
                                    }
                                } else {
                            ...
                            for(int i = 0;i<data[j].length;i++){
                                if(data[j][i].getCandClass() >= 0 ){
                                    ...
                                    if(data[j][i].getCandClass() == 4){
                                        if(data[j][i].beenViewed()) addPoint(14,plotType.getXval(data[j][i]),plotType.getYval(data[j][i]),false);
                                        else addPoint(15,plotType.getXval(data[j][i]),plotType.getYval(data[j][i]),false);
                                    }else{
                                        if(master.show(data[j][i].getHarmonicType())){
                                            if(master.mark(data[j][i].getHarmonicType())){
                                                ...
                                                if(data[j][i].beenViewed())
                                                    addPoint(5+j,plotType.getXval(data[j][i]),plotType.getYval(data[j][i]),false);
                                                else addPoint(j,plotType.getXval(data[j][i]),plotType.getYval(data[j][i]),false);
                                            }
                                        }
                                    }
                                } else{
    ...
    private void markReleasedListener(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_markReleasedListener
        ...
        if ( evt.getButton() == evt.BUTTON1) {
            ...
            if(clicking) this.clickArea(evt.getX(),evt.getY());
        }
