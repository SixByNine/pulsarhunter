...
public class DataLibrary {
    ...
    protected void setCandListGroup(String name, Hashtable<String, CandListHeader> group, Hashtable<String, File> files) {
        ...
        for (String key : group.keySet()) {
            if (!this.coverage.containsKey(name)) {
                this.coverage.put(name, new Hashtable<String, Coordinate[]>());
            }
            if (!this.data.containsKey(name)) {
                this.data.put(name, new Hashtable<String, File>());
            }
    ...
    public void importClistsFromDir(final File file, String group, final LoadingSplash corelsplash) {
        ...
        if (fList == null) {
            ...
            for (final File f : fList) {
                ...
                if (f.isDirectory()) {
                    if (group == null) {
                        importClistsFromDir(f, f.getName(), lsplash);
                    } else {
                        importClistsFromDir(f, group, lsplash);
                    }
    ...
    private void importClistFromFile(final File f, String group) {
        try {
            ...
            if (cHead == null) {
                ...
                if (!this.coverage.containsKey(group)) {
                    this.coverage.put(group, new Hashtable<String, Coordinate[]>());
                }
                if (!this.data.containsKey(group)) {
                    this.data.put(group, new Hashtable<String, File>());
                }
    ...
    public Reader getReader(File file) throws IOException {
        if (file.getName().endsWith(".clist.gz")) {
    ...
    public CandList getCandList(String name, String groupname) {
        ...
        if (infile == null) {
            throw new RuntimeException("Cannot load Data " + name + ", data file not in this data library");
        }
        ...
        for (String gname : groups.keySet()) {
            for (String cname : groups.get(gname).keySet()) {
                if (cname.contains(search)) {
                    result.add(new Pair<String, String>(gname, cname));
                }
            }
        }
    ...
    public ArrayList<KnownPSR> getKnownPulsars() {
        if (this.knownPSR == null) {
            ...
            this.knownPSR = getKnownPulsarsFromFile(pulsarFile);
        }
        return this.knownPSR;
    }

    protected ArrayList<KnownPSR> getKnownPulsarsFromFile(File pulsarFile) {
        ArrayList<KnownPSR> kpsr = new ArrayList<KnownPSR>();
        System.out.println(pulsarFile.getAbsolutePath());
        ...
        if (pulsarFile.exists()) {
            try {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (period == 0 || name.equals("") || ra == null || dec == null) {
                        continue;
                    }
                    Coordinate coord = new Coordinate(ra, dec);
                    kpsr.add(new KnownPSR(name, coord, period, dm));
    ...
    public void checkCandListForKnownPSRs(final CandList clist) {
        ...
        for (KnownPSR kp : this.getKnownPulsars()) {
            if (cc.difference(clist.getBeam().getCoord(), kp.getPosition()) > this.getOptions().getDistmax()) {
                continue;
            }
